...
namespace MyDay
{
    public partial class Projects : FormBase
    {
        public Projects()
        {
            InitializeComponent();
        ...
        private void PopulateList()
        {
            try
            {
                ...
                {
                    var query = from ts in db.Projects
                                select new { Project = ts.ProjectCode };
            ...
            try
            {
                ...
                if (!String.IsNullOrEmpty(item))
                {
                    using (MyDayData db = new MyDayData())
                    {
                        ...
                        var query = from ts in db.Projects
                                    where ts.ProjectCode == item
                                    select new { Project = ts.ProjectCode };

                        if (query.FirstOrDefault() == null)
                        {
                            Project newItem = new Project { ProjectCode = item };
                            db.AddToProjects(newItem);
                            db.SaveChanges();
        ...
        private void Delete()
        {
            try
            {
                if (dgvItems.SelectedRows.Count > 0)
                {
                    ...
                    {
                        var query = from ts in db.Projects
                                    where selections.Contains(ts.ProjectCode)
                                    select ts;

                        if (query.FirstOrDefault() != null)
                        {
                            foreach (var row in query)
                                db.Projects.DeleteObject(row);
